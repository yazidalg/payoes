...
The Stellar public key where customer payments are sent. Configured during onboarding and editable in **Settings → Settlement Wallet**.
...
Checkout sessions are created automatically when you finalize an invoice or when a customer visits a payment link. You can list and retrieve them via the API.
