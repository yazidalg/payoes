...
import { GithubRepoLink } from "/snippets/github-link";
...
Set a bearer token in `.env.local` at the repository root or your production environment:
...
For VPS setup, use the scripts in <GithubRepoLink path="tree/main/scripts/cron">`scripts/cron`</GithubRepoLink> at the repository root (`run-settlement.sh`, `run-webhook-retries.sh`, and `README.md`).
