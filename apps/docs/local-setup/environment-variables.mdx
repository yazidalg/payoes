...
description: Reference for `.env.local` at the repository root when running Payoes locally.
---
...
Self-hosted deployments only. Workers start automatically when the Next.js server boots (`instrumentation.ts`).

| Variable                    | When to set          | Description                                                                 |
| --------------------------- | -------------------- | --------------------------------------------------------------------------- |
| `ENABLE_SETTLEMENT_STREAM`  | Optional             | `true` / `false`. Default: on in production, off in development. Streams Horizon payments to detect escrow QR deposits. Requires a single web replica. |

See [Webhook retry worker](/local-setup/webhook-retry-worker) for retry behavior.
...
| Variable                 | Required    | Description                                                                                              |
| ------------------------ | ----------- | -------------------------------------------------------------------------------------------------------- |
| `NEXT_PUBLIC_APP_URL`    | Recommended | Public Payoes app URL. Local: `http://localhost:3000`. Leave empty on VPS to use the request host for auth |
| `NEXT_PUBLIC_DOCS_URL`   | Recommended | URL opened by dashboard documentation links. Local: `http://localhost:3001`                              |
| `NEXT_PUBLIC_GITHUB_URL` | Recommended | Public GitHub repository URL for marketing and docs links. Default: `https://github.com/yazidalg/payoes` |
...
NEXT_PUBLIC_GITHUB_URL=https://github.com/yazidalg/payoes
```
