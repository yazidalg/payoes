...
export function Users6({
  "data-hovered": hovered,
  ...rest
}: { "data-hovered"?: boolean } & SVGProps<SVGSVGElement>) {
  const centerHeadRef = useRef<SVGCircleElement>(null);
  const leftHeadRef = useRef<SVGCircleElement>(null);
  const rightHeadRef = useRef<SVGCircleElement>(null);
  const bodyRef = useRef<SVGPathElement>(null);

  useEffect(() => {
    if (!hovered) return;

    centerHeadRef.current?.animate(
      [
