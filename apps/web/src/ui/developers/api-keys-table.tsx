...
export function ApiKeysTable({
  ...
  refreshKey?: number;
  onCreateClick?: () => void;
  onRowClick?: (apiKey: ApiKeyRow) => void;
    ...
    () => [
      {
        ...
        cell: ({ row }: { row: Row<ApiKeyRow> }) => (
          <span className="flex items-center gap-2">
            ...
            <span
              className={cn(
                row.original.revokedAt && "text-neutral-400 line-through",
              )}
            >
              {row.original.name}
            </span>
      ...
      },
      {
        ...
        minSize: 120,
        cell: ({ row }: { row: Row<ApiKeyRow> }) =>
          scopesToName(row.original.scopes ?? ["apis.all"]).name,
      },
      {
        id: "environment",
        header: "Environment",
        ...
        minSize: 120,
        cell: ({ row }: { row: Row<ApiKeyRow> }) => (
          <span className="capitalize">{row.original.environment}</span>
        ),
      },
      {
        id: "createdAt",
      ...
      },
      {
        ...
        cell: ({ row }: { row: Row<ApiKeyRow> }) =>
          row.original.lastUsedAt
            ? timeAgo(new Date(row.original.lastUsedAt))
            : "Never",
      },
      {
        id: "menu",
    ...
    <div className="grid grid-cols-1">
      {isLoading ? (
        ...
        <TableEmptyState
          ...
          description="No API keys have been created for this organization yet."
