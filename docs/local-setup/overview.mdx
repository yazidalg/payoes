...
| **Docs** (optional) | Mintlify preview on port 3001 | `npm run docs:dev` |
...
<AccordionGroup>
  ...
  <Accordion title="Optional — only if you need these features">
    - **Google OAuth credentials** — sign in with Google (email/password demo login works without this)
    - **SMTP settings** — send real team invitation emails
...
<Steps>
  ...
  <Step title="KYC verification (optional)">
    If you need to test production mode, continue with [KYC verification](/local-setup/kyc-verification).
  </Step>
</Steps>

<Note>
  You do **not** need Persona to develop sandbox payments, checkout, customers, or webhooks. Those features work on Stellar testnet with the default local setup.
</Note>
