...
| `AUTH_DEMO_EMAIL` | Recommended | Email for password-based demo login |
| `AUTH_DEMO_PASSWORD` | Recommended | Password for demo login |

<Tip>
  For local development, demo login is the fastest path. Add Google OAuth only when you need to test the Google sign-in flow.
</Tip>
...
| `NEXT_PUBLIC_PERSONA_INQUIRY_TEMPLATE_ID` | Optional | Client-side template ID if needed by SDK |
...
In [Google Cloud Console](https://console.cloud.google.com/), set the authorized redirect URI to:
